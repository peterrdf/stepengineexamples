...
function setTreeTab(treeType, handle) {
    ...
    $('#tree-labels').append(
        $('<a>', {
            ...
            console.log(`Switching to tree type: ${treeType}`); //#todo
            //showTree(treeType);
        })
    );
    ...
    if (isAutoNavigate()) {
        if (containsGeometry(handle)) {
            if (getTreeItemOwnVisibleItem(handle) === selectedGInstanceIndex) {
                ...
                const hanlde = getHandle(option);
                printPropertySets(hanlde);
        ...
        if (parentOption.exists()) {
            ...
            if ([FULL_VISIBLE, EMPTY_VISIBLE].indexOf(visibilityClass) === -1) {
                ...
                if (visibleItemIndex !== -1) {
                    ...
                    } else if (g_viewer._id2instance[visibleItemIndex]?.visible) {
...
function getVisibilityWhenMultiChildren(visibleIndexes) {
    ...
    const hasSomeChildVisible = visibleIndexes.some(
        (index) => g_viewer._id2instance[index]?.visible
    );
    return hasSomeChildVisible ? PARTIAL_VISIBLE : EMPTY_VISIBLE;
}
...
function set3DVisibility(option) {
    ...
    if (treeItemVisibleItems.length) {
        ...
        const allTreeItemVisibleItems = [];
        treeItemVisibleItems.forEach((index) => {
            allTreeItemVisibleItems.push(index);
            const instance = g_viewer._id2instance[index];
            if (instance.childrenIds && instance.childrenIds.length > 0) {
                allTreeItemVisibleItems.push(...instance.childrenIds);
            }
        });

        const isVisible = !g_viewer._id2instance[treeItemVisibleItems[0]]?.visible;
        allTreeItemVisibleItems.forEach(
            (index) => (g_viewer._id2instance[index].visible = isVisible));
...
function setParentVisibility(option, visibilityClass) {
    ...
    if (parentOption.parent(ITEM_CHILDREN_CONTAINER_TAG).attr('id') !== treeId(activeTree)) {
...
function calcVisibilityClass(defaultVisibility, handle) {
    if (defaultVisibility !== PARTIAL_VISIBLE) {
        ...
        if (own3DIndex !== -1) {
            const isOwn3DVisible = g_viewer._id2instance[own3DIndex]?.visible;
            if (
...
function deselectItemIn3DView(option) {
    ...
    if (treeItemVisibleItems.length && g_viewer._selectedInstances.length) {
        const selectedInstances = [];

        g_viewer._selectedInstances.forEach((id) => {
            if (!treeItemVisibleItems.includes(id)) {
                selectedInstances.push(id);
            }
        })
        g_viewer._selectedInstances = selectedInstances;
    }
}
...
function displayInTree() {
    ...
    if (g_viewer._selectedInstances.length) {
        const rootHandle = Module.GetTreeHeadHandle(activeTree);
        const instance = g_viewer._id2instance[g_viewer._selectedInstances[0]];

        if (g_viewer._id2instance[instance.parentId]) {
            selectedGInstanceIndex = instance.parentId;
        } else {
            selectedGInstanceIndex = instance.id;
        }
...
// Svilen - 3D View supports single selection only
...
function areAllIndexesVisible(visibleIndexes) {
    return visibleIndexes.every((index) => {
        return g_viewer._id2instance[index]?.visible;
    })
}
...
function hideAllTreeItems() {
    ...
    Object.entries(g_viewer._id2instance).forEach(([id, instance]) => {
...
function hideSelectedTreeItems() {
    ...
    g_viewer._selectedInstances.forEach((id => {
        g_viewer._id2instance[id].visible = false;
    }));
...
function presetOptionAsVisible(option) {
    ...
    if (treeItemVisibleItems.length) {
        g_viewer._id2instance[treeItemVisibleItems[0]].visible = true;
    }
}
...
function highlightIn3D(option, zoom = false) {
    ...
    if (visibleIndexes.length) {
        ...
        const allVisibleIndexes = [];
        for (let i = 0; i < visibleIndexes.length; i++) {
            const instance = g_viewer._id2instance[visibleIndexes[i]];
            allVisibleIndexes.push(instance.id);
            if (instance.childrenIds?.length) {   
                allVisibleIndexes.push(...instance.childrenIds);
            }
        }

        g_viewer._selectedInstances = allVisibleIndexes;

        if (zoom) {
            const instanceId = visibleIndexes[0];
...
function hasSomeHiddenInstances() {
    const visibleInstances = Object.values(g_viewer._id2instance).filter((inst) => inst.visible);
    return Object.keys(g_viewer._id2instance).length !== visibleInstances.length;
}
