...
void	createIfcBRepShape(SHELL * shell)
{
	...
	sdaiAppend(aggrRepresentations, sdaiINSTANCE, (void*) buildShapeRepresentationInstance(BODY_BREP_REPRESENTATION, shell));
}
				...
				while (faces) {
					...
					while ((faces) &&
						...
						while (outerPolygon) {
							...
							sdaiAppend(aggrPolygon, sdaiINSTANCE, (void*) point->ifcCartesianPointInstance);
